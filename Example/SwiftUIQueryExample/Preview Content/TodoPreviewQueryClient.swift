//
//  TodoPreviewQueryClient.swift
//  SwiftUIQueryExample
//
//  Created by Jakub Łaptaś on 01/01/2023.
//

import Foundation
import SwiftUIQuery

private let staticTodosData = """
...
final class TodosPreviewQueryClient: QueryClient {
  func query<Result>(_ request: URLRequest) async throws -> Result where Result: Decodable {
    return try decoder.decode(Result.self, from: staticTodosData.data(using: .utf8)!)
  }
}
